...
import { Home, Play, PlusCircle, User, MoreHorizontal } from 'lucide-react';
...
export function BottomNav() {
  ...
  const tabs = [
    ...
    { to: '/more' as const, icon: MoreHorizontal, label: 'More' },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 px-4 pt-2" style={{ paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom))' }}>
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200 ${
                special ? 'text-primary' : isActive ? 'neu-pressed text-primary' : 'text-muted-foreground'
              }`}
            >
